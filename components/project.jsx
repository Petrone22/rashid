...
export default function Project({
  ...
  return (
    ...
    >
      <section className="rounded-lg bg-gray-100 dark:bg-white/10 hover:dark:bg-white/20 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 relative text-start dark:text-white sm:h-[20rem]  hover:bg-gray-200 transition">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col gap-2 h-full sm:group-even:ml-[18rem]">
          ...
          <p className="mt-2 leading-relaxed text-gray-700 text-center sm:text-start dark:text-white/70 ">
            {description}
          </p>
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto justify-center sm:justify-normal ">
            {tags.map((tag, index) => (
              <li
                className="bg-black/[0.7] dark:text-white/70 px-3 py-1 text-[0.7rem] tracking-wider text-white rounded-full"
                key={index}
              >
                {tag}
              </li>
